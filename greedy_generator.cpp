...
int main(int argc, char** argv) {
    ...
    P = atoi(argv[2]);
    if(argc!=3){
        cout<<"Usage: " << argv[0] << "no_of_states, alphabet_size \n";
        return 1;
    }

   //  cin >> N >> P;
    string file_name = "greedy_log_" + to_string(N) + "_" + to_string(P) + ".txt";
    ...
    do{
        ...
        for (int i = 0; i < P; ++i) {
         ...
         isSync = syncCheck(automata, inv_automata_ptrs, inv_automata, N, P);
        delete [] inv_automata;
        delete [] inv_automata_ptrs;
    }
    while(!isSync);
    PNode *path = nullptr;
    unsigned long long int fir_lenght =  shortestPath(automata,N,P,path);
    unsigned long long int prev = fir_lenght;

    for(int i =0; i<P ; i++){
        for(int j=i; j<N*P; j += P){
            unsigned long long int next;
            int re_val = automata[j];
            automata[j] = int(floor(j/P));
            bool revert = false;
            int* inv_automata_ptrs = new int[P * (N + 1)];
            int* inv_automata = new int[P * N];
            for (int k = 0; k < P; ++k) {
            ...
            revert = syncCheck(automata,inv_automata_ptrs,inv_automata,N,P);
            path = nullptr;
            next = shortestPath(automata, N, P, path);
            if((!revert) || prev > next){
                automata[j] = re_val;
            }else{
                prev = next;
